 ...
 * violation.
 *
 * tests/static-check.mjs proves the policy is PRESENT and self-consistent, and
 * tests/bundle.mjs proves the bundle boots — but both run in Node, where no
...
if (!existsSync(join(SITE, "app.html"))) {
  console.error("No _site — run `node tools/build.mjs _site` first.");
  process.exit(1);
}
