#!/usr/bin/env python3
"""
Regenerates assets/social/og-card.png — the 1200x630 Open Graph card.

    python tools/build/build-og-card.py      # writes assets/social/og-card.png
...
OUT = ROOT / "assets" / "social" / "og-card.png"
...
def clipboard_icon(d: ImageDraw.ImageDraw, x: int, y: int, w: int, h: int) -> None:
    """The app icon, redrawn rather than scaled — assets/icons/icon.svg is a stroked
...
def main() -> int:
    ...
    ap.add_argument("--check", action="store_true",
                    help="exit non-zero if assets/social/og-card.png is out of date")
    args = ap.parse_args()
